...
class Ndpm(nn.Module):
    ...
    def learn(self, x, y, step):
        ...
        if self.config.get('send_to_stm_always'):
            ...
            # Debugging prints for data distribution and NLLs
            if True: # Force print every step for tuning
                unique, counts = torch.unique(destination, return_counts=True)
                dist_str = ", ".join([f"{'STM' if u==0 else f'Expert {u-1}'}: {c.item()}" for u, c in zip(unique, counts)])
                
                # Compare NLL values to understand branching
                with torch.no_grad():
                    mean_nlls = nll.mean(dim=0)
                    nll_str = ", ".join([f"{'STM' if i==0 else f'Exp {i-1}'}: {mean_nlls[i]:.1f}" for i in range(len(mean_nlls))])
                
                print(f"[NDPM Learn] Step {step} distribution: {dist_str} | STM: {len(self.stm_x)}/{self.stm_capacity}")
                print(f"            NLLs: {nll_str} (log_alpha: {self.config.get('log_alpha')})")
            ...
            if loss.requires_grad:
                ...
                for k, usage in enumerate(expert_usage):
                    if usage > update_threshold:
                        if k > 0:
                            print(f"[NDPM Learn] Updating Expert {k-1} (soft usage: {usage:.2f})")
    ...
    def sleep(self, dream_dataset):
        new_expert_id = len(self.experts) - 1 # Since placeholder is 0
        print(f"\n[NDPM Sleep] STM full ({len(self.stm_x)} samples). BIRTHING NEW EXPERT {new_expert_id}...")
