"""Run all pipeline combinations experiment module for SML.

Reads a config file (same format as run_pipeline_sml.py) where any parameter
value can be a list of alternatives. This script generates the cartesian
product of all such lists and invokes run_pipeline_sml.py once per combination.
"""
...
# Path to run_pipeline_sml.py relative to this script
PIPELINE_SCRIPT = Path(__file__).parent / "run_pipeline_sml.py"
...
def run_pipeline_for_config(config: dict, combo_index: int, total: int, extra_args: list):
    """Write a temporary config and invoke run_pipeline_sml.py as a subprocess."""
    tmp_config_path = Path("/tmp") / f"_run_all_pipelines_sml_combo_{combo_index}.yaml"
    ...
    if result.returncode != 0:
        logger.warning(
            f"run_pipeline_sml.py exited with code {result.returncode} "
...
def get_predicted_run_id(config: dict) -> str:
    """
    Replicates the run_id generation logic from run_pipeline_sml.py.
    """
    ...
    # Construct ID exactly as run_pipeline_sml.py does
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run all combinations of hyperparameters for SML."
        )
    )
    parser.add_argument(
        "--config",
        type=str,
        required=True,
        help="Path to the YAML configuration file (same format as run_pipeline_sml.py).",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        default=False,
        help="If set, print all combinations but do NOT actually run run_pipeline_sml.py.",
    )
    parser.add_argument(
        "--skip-existent",
        action="store_true",
    ...
    # Capture any extra args (like --server-ip, --port) to forward to run_pipeline_sml.py
