"""Run SML unified experiment module."""
...
def parse_exp_args(str_args=None):
    ...
    parser = argparse.ArgumentParser(description="unified SML remote experiments")
    parser.add_argument("--server_ip", required=True, help="IP of the SML server")
    parser.add_argument("--port", type=int, default=5555, help="Port of the SML server")
    ...
    parser.add_argument("--dpmm-type", choices=DPMM_MODEL_TYPE, default="diagonal")
...
def run_exp(args, other_args=None, _suppress_output=False):
    ...
    run_id = f"sml_{args.dataset}_{args.model}"
    if args.model == "dpmm":
